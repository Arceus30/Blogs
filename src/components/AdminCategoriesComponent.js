...
export default function Categories({ page }) {
        ...
        const fetch = async () => {
            ...
            try {
                const response = await api.get(
                    `${process.env.NEXT_PUBLIC_ADMIN_CATEGORIES_API}?page=${page}&limit=${categoriesLimit}`,
                );
    ...
    return (
        <>
            <div className="grow grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 grid-rows-6 sm:grid-rows-7 lg:grid-rows-9 gap-3 mt-2">
                {categories?.map((cat, idx) => (
                    <div
                        key={cat?._id || idx}
                        className="flex items-center justify-between hover:bg-gray-300 rounded-xl gap-2 px-2"
                    >
                        <Link
                        ...
                        >
                            <h3 className="text-base md:text-lg lg:text-xl font-semibold hover:font-bold">
                        ...
                        <div className="flex gap-1">
                            {cat?.slug !== "general" && (
                                <>
                                    <Link
                                        className="bg-yellow-500 text-white font-medium p-1 lg:p-2 rounded-xl border cursor-pointer hover:bg-yellow-700"
                                        href={`${process.env.NEXT_PUBLIC_ADMIN_UPDATE_CATEGORY}/${cat?.slug}`}
                                    >
                                        <MdEdit />
                                    </Link>
                                    <button
                                        className="bg-red-500 text-white font-medium p-1 lg:p-2 rounded-xl border cursor-pointer hover:bg-red-700"
                                        onClick={() =>
